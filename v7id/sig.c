...
int *p;
{
	...
	if(rp->p_sig != SIGKIL)
		rp->p_sig = sig;
/***/	if(rp->p_pri > PUSER && rp->p_pri < 128)
		rp->p_pri = PUSER;
	if(rp->p_stat == SWAIT)
		setrun(rp);
}
...
psig()
{
	...
	if((p=u.u_signal[n]) != 0) {
		u.u_error = 0;
/***/		if(n != SIGSYS && n != SIGINS && n != SIGTRC)
			u.u_signal[n] = 0;

/***/		aps = u.u_ar0[RPS];
/***/		asp = u.u_ar0[RSP];
/***/		if (u.u_nostack) {
/***/			/* No user stack -- set up stack for signal routine */
/***/			u.u_nostack = 0;
/***/			aps =| (1<<31);
/***/			n = (14<<16) + ((SSIZE-2)<<8) - 16;
/***/		} else {
/***/			n = asp - 12;
/***/		}
/***/
...
/*** doesn't do anything until we figure out how ***/
...
procxmt()
{
	...
	/* write u */
	case 6:
		p = &u.inta[ipc.ip_addr>>2];	/***/
		if (p >= u.u_fsav && p < &u.u_fsav[24])		/***/
			goto ok;
		for (i=0; i<18; i++)			/***/
	...
	ok:
		if (p == &u.u_ar0[RPS]) {
			/* PSW : allow only cc & af bits to be changed */
			ipc.ip_data =& 010017;			/***/
			ipc.ip_data =| (*p & ~010017);		/***/
		}
		*p = ipc.ip_data;
		break;
